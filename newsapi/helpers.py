...
from newsapi.models import NewsTimeSeries
import yake
...
def clean_db():
    ...
    for each in queryset:
        ...
        # cleaning scrap from text
        title = clean(str(each["title"]),
                      fix_unicode=True,  # fix various unicode errors
                      to_ascii=True,  # transliterate to closest ASCII representation
                      lower=True).strip(" ")
        content = clean(str(each["content"]),
                      fix_unicode=True,  # fix various unicode errors
                      to_ascii=True,  # transliterate to closest ASCII representation
                      lower=True).strip(" ")
        description = clean(str(each["description"]),
                      fix_unicode=True,  # fix various unicode errors
                      to_ascii=True,  # transliterate to closest ASCII representation
                      lower=True).strip(" ")

        # removing publication and author from news
        ss = each["publication__name"].split(" ")
        for i in ss:
            title = title.replace(i, "")
            content = content.replace(i, "")
            description = description.replace(i, "")

        NewsTimeSeries.objects.filter(
            id=each["id"],
        ).update(
            title = title,
            content = content,
